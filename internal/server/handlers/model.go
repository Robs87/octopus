...
func getModelList(c *gin.Context) {
	...
	// modelEntry: 对外暴露的模型名 + 分组配置的上下文长度
	type modelEntry struct {
		name          string
		contextLength int
	}
	models := lo.Map(groups, func(g model.Group, _ int) modelEntry {
		return modelEntry{name: g.Name, contextLength: g.ContextLength}
	})
	...
	if apiKey.SupportedModels != "" {
		...
		models = lo.Filter(models, func(m modelEntry, _ int) bool {
			return lo.Contains(supportedModels, m.name)
		})
	}
	...
	if c.GetString("request_type") == "anthropic" {
		...
		for _, m := range models {
			openAIModels = append(openAIModels, model.OpenAIModel{
				...
				Created:          1763395200,
				OwnedBy:          "octopus",
				MaxContextLength: m.contextLength,
			})
		}
		c.JSON(200, gin.H{
			"success": true,
