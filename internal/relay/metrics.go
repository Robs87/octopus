...
type RelayMetrics struct {
	...
	// 最终尝试使用的渠道 key 展示名（备注优先，否则 key 本体）；run() 每次尝试都会覆盖，
	// 最后一次尝试即为最终渠道，与 finalChannel 结论一致。
	UsedKeyName string
}
...
func (m *RelayMetrics) saveLog(ctx context.Context, err error, duration time.Duration, attempts []model.ChannelAttempt, channelID int, channelName string) {
	relayLog := model.RelayLog{
		...
		Username:         op.UserGet().Username, // 调用方用户名（后台管理员登录名）
		LogType:          model.LogTypeRelay,    // 转发日志
	}
	...
	// 渠道日志中 API Key 列应展示"渠道实际使用的 key"（备注优先，否则 key 本体），
	// 与渠道测试日志口径一致；只有在取不到渠道 key 时（如全链路被跳过）才回退调用方 API Key 名称。
	if m.UsedKeyName != "" {
		relayLog.RequestAPIKeyName = m.UsedKeyName
	}
