...
func (r *relayRun) run() {
	...
	for r.iter.Next() {
		...
		attempts, err := r.prepareAttempts()
		if err != nil {
			lastErr = err
			continue
		}
...
func (r *relayRun) prepareAttempts() ([]*relayAttempt, error) {
	...
	attempts := make([]*relayAttempt, 0, len(keys))
	for i, usedKey := range keys {
		...
		log.Infof("request model %s, mode: %d, forwarding to channel: %s model: %s key: %d/%d (attempt %d/%d, sticky=%t)",
			r.metrics.RequestModel, r.group.Mode, channel.Name, item.ModelName,
			i+1, len(keys), r.iter.Index()+1, r.iter.Len(), r.iter.IsSticky())

		attempts = append(attempts, &relayAttempt{
			relayRun:   r,
...
func (ra *relayAttempt) run() (bool, error) {
	...
	ra.usedKey.LastMessage = fwdErr.Error()
	if len(ra.upstreamErrBody) > 0 {
		ra.usedKey.LastMessage = string(ra.upstreamErrBody)
	}
