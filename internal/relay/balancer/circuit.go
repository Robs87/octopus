...
// halfOpenProbeTimeout 半开试探允许的最大等待时间。
// 如果进入 HalfOpen 后试探请求没有在限定时间内上报结果（例如被跳过/请求被取消），
// 允许重新放行一个试探请求，避免熔断器永久卡在半开状态。
...
type circuitEntry struct {
	...
	HalfOpenSince       time.Time // 进入 HalfOpen 的时间，用于超时重新放行试探
...
func IsTripped(channelID, keyID int, modelName string) (tripped bool, remaining time.Duration) {
	...
	case StateHalfOpen:
		// 已有试探请求在进行中，拒绝其他请求；
		// 但如果试探请求超时未上报结果（被跳过/被取消），重新放行一个试探请求，避免永久卡死。
		if time.Since(entry.HalfOpenSince) > halfOpenProbeTimeout {
			entry.HalfOpenSince = time.Now()
			log.Warnf("circuit breaker [%s] HalfOpen probe timeout, allowing new probe", key)
			return false, 0
		}
		return true, 0
