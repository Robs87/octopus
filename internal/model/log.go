...
// 日志类型
const (
	LogTypeRelay     = "relay"      // 转发日志（分组调用）
	LogTypeTest      = "test"       // 渠道测试日志
	LogTypeGroupTest = "group_test" // 分组测试日志
)
...
type RelayLog struct {
	...
	Username          string           `json:"username"`                                 // 调用方用户名（后台管理员登录名）
	LogType           string           `json:"log_type"`                                 // 日志类型：relay=转发日志，test=渠道测试日志
}
