...
function MorphingDialogContent({
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        // 有嵌套 Radix Dialog（如编辑额度弹窗）打开时，Escape 由内层 Dialog 消费，
        // 外层 MorphingDialog 不应跟着关闭
    ...
    // 用 capture 阶段监听：先于内层 Radix Dialog（bubble 阶段）执行，
    // 这样按 Escape 时内层 dialog-content 尚未卸载，querySelector 能检测到，
    // 避免内层弹窗被 Escape 关闭时外层 MorphingDialog 跟着关闭
  ...
  useClickOutside(
    ...
    (event) => {
      ...
      // 嵌套 Radix Dialog（如编辑额度弹窗）通过 Portal 渲染在容器外，
      // 点击其内部或遮罩不应关闭外层 MorphingDialog
