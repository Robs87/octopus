        ...
        <div className="w-screen max-w-full md:max-w-4xl h-[calc(100vh-2rem)] min-h-0 flex flex-col">
            ...
            <MorphingDialogDescription className="flex-1 min-h-0 overflow-hidden">
                <GroupEditor
                    ...
                    onSubmit={({ name, match_regex, mode, context_length, first_token_time_out, session_keep_time, members }) => {
                        ...
                        createGroup.mutate(
                            { name, mode, match_regex: match_regex ?? '', context_length: context_length ?? 0, first_token_time_out: first_token_time_out ?? 0, session_keep_time: session_keep_time ?? 0, items },
                            {
