...
import { Trash2, X, Pencil, FlaskConical, Loader2 } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { toast as sonnerToast } from 'sonner';
import { type Group, useDeleteGroup, useUpdateGroup, useTestGroup } from '@/api/endpoints/group';
            ...
            <MorphingDialogDescription className="flex-1 min-h-0 overflow-hidden">
                <GroupEditor
                    ...
                    initial={{
                        ...
                        mode: group.mode,
                        context_length: group.context_length ?? 0,
...
export function GroupCard({ group }: { group: Group }) {
    ...
    const handleSubmitEdit = useCallback((values: GroupEditorValues, onDone?: () => void) => {
        ...
        const nextContextLength = values.context_length ?? 0;
        ...
        if (nextContextLength !== (group.context_length ?? 0)) payload.context_length = nextContextLength;
    ...
    }, [group.context_length, group.first_token_time_out, group.session_keep_time, group.id, group.items, group.match_regex, group.mode, group.name, onSuccess, onError, updateGroup]);
    ...
    const handleTestGroup = useCallback(() => {
        ...
        testGroup.mutate(group.id, {
            onSuccess: (data) => {
                if (data.success) {
                    // 成功提示：硬编码中文，不依赖 i18n 解析（与渠道测试一致）
                    toast.success(
                        '测试成功',
                        {
                            description: `分组「${group.name}」· 渠道 ${data.channel_name} · 模型 ${data.model_name} · 耗时 ${((data.latency_ms ?? 0) / 1000).toFixed(2)} 秒`,
                            position: 'top-left',
                        }
                    );
                    // 上下文长度比对：仅当上游返回了模型上下文长度，且与分组设置不一致时提示。
                    // 注意：group.context_length 与 model_context_length 均为 tokens 单位，比较时先 round 到 k 避免舍入误报。
                    if (data.model_context_length && data.model_context_length > 0) {
                        const groupCtxLenK = Math.round((group.context_length ?? 0) / 1000); // tokens → k
                        const upstreamCtxLenK = Math.round(data.model_context_length / 1000); // tokens → k
                        if (upstreamCtxLenK !== groupCtxLenK) {
                            sonnerToast(
                                `上下文长度不一致`,
                                {
                                    description: `分组「${group.name}」设置为 ${groupCtxLenK}k，测试模型 ${data.model_name} 的上下文长度为 ${upstreamCtxLenK}k，是否更新？`,
                                    ...
                                    action: {
                                        ...
                                        onClick: () => {
                                            updateGroup.mutate(
                                                { id: group.id!, context_length: data.model_context_length },
                                                {
    ...
    }, [group.id, group.name, group.context_length, testGroup, updateGroup, t]);
                    ...
                    <Tooltip side="top" sideOffset={10} align="center">
                        <TooltipTrigger asChild>
                            <button
                                ...
                                disabled={testGroup.isPending || !group.id}
                                aria-label={t('detail.actions.test')}
                        ...
                        <TooltipContent>{t('detail.actions.test')}</TooltipContent>
                    </Tooltip>
