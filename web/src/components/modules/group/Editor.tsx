...
export function GroupEditor({
    ...
    // 上下文长度以 k 为单位展示（如 128 = 128k tokens），提交时转回 token 数；新建默认 256k
    const [contextLengthK, setContextLengthK] = useState<number>(Math.round((initial?.context_length ?? 256000) / 1000));
    ...
    const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
        ...
        onSubmit({
            ...
            mode,
            context_length: Math.round(contextLengthK * 1000),
            ...
            <div className="flex-1 min-h-0 overflow-hidden pr-1">
                <FieldGroup className="gap-4 flex flex-col min-h-0 h-full">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                        ...
                        <Field>
                            <FieldLabel htmlFor="group-context-length">
                                {t('form.contextLength')}
                                <TooltipProvider>
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <HelpCircle className="size-4 text-muted-foreground cursor-help" />
                            ...
                            <div className="relative">
                                <Input
                                    id="group-context-length"
                                    type="number"
                                    inputMode="numeric"
                                    min={0}
                                    step={1}
                                    value={String(contextLengthK)}
                                    onChange={(e) => {
                                        const raw = e.target.value;
                                        if (raw.trim() === '') {
                                            setContextLengthK(0);
                                        ...
                                        const n = Number.parseInt(raw, 10);
                                        setContextLengthK(Number.isFinite(n) && n > 0 ? n : 0);
                                    }}
