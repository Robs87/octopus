...
// 编辑界面金额固定显示小数点后 4 位
const formatMoney4 = (n?: number) => (n ?? 0).toFixed(4);

// 将 key 最近返回详情中常见的英文提示转换为中文
function translateLastMessage(msg?: string): string {
    if (!msg) return '无返回详情';
    const text = msg.trim();
    ...
    const exactMap: Record<string, string> = {
        ...
        'exceeded your current quota': '余额不足',
    ...
    if (lower.includes('insufficient_quota') || lower.includes('insufficient quota') || lower.includes('insufficient balance')) {
        return '余额不足';
    }
    ...
    if (lower.includes('internal server error') || lower.includes('server error') || lower.includes('bad gateway') || lower.includes('service unavailable')) {
        return '服务异常';
    }
    if (lower.includes('model not found') || lower.includes('model does not exist')) {
        return '模型不存在';
    }
...
export function CardContent({ channel, stats }: { channel: Channel; stats: StatsMetricsFormatted }) {
    ...
    const handleUpdate = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const keys_to_add = nextKeys
            ...
            .map((k) => ({ enabled: k.enabled, channel_key: k.channel_key, quota: k.quota ?? 0, remark: k.remark ?? '' }));
        ...
        const keys_to_update = nextKeys
            ...
            .map((k) => {
                ...
                const u: { id: number; enabled?: boolean; channel_key?: string; quota?: number; remark?: string } = { id: k.id as number };
                if (k.enabled !== orig.enabled) u.enabled = k.enabled;
                if (k.channel_key !== orig.channel_key) u.channel_key = k.channel_key;
                if ((k.quota ?? 0) !== (orig.quota ?? 0)) u.quota = k.quota ?? 0;
                if ((k.remark ?? '') !== orig.remark) u.remark = k.remark ?? '';
                return Object.keys(u).length > 1 ? u : null;
            })
            .filter((u) => u !== null) as Array<{ id: number; enabled?: boolean; channel_key?: string; quota?: number; remark?: string }>;
                                ...
                                <dl className="grid gap-3 grid-cols-1 sm:grid-cols-3">
                                    ...
                                    <div className="rounded-2xl border bg-linear-to-br from-chart-5/10 to-chart-5/5 p-3 sm:p-4">
                                        ...
                                        <dd className="text-xl sm:text-2xl font-bold text-chart-5">
                                            {formatMoney4(stats.total_cost.raw)}
                                                        ...
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                <Badge
                                                                    variant="secondary"
                                                                    className={cn(
                                                                        "h-5 px-1.5 text-[10px] cursor-help",
                                                                        key.status_code === 200
                                                                            ? "bg-green-500/15 text-green-700 dark:text-green-400"
                                                                            : key.status_code === 401 ||
                                                                                key.status_code === 403 ||
                                                            ...
                                                            <TooltipContent>
                                                                {translateLastMessage(key.last_message)}
                                                            </TooltipContent>
                                                        </Tooltip>
                                                    )}

                                                    <Badge variant="secondary" className="h-5 px-1.5 text-[10px]">
                                                        {formatMoney4(key.total_cost)}$
                                                        {key.quota > 0 && (
                                                            <span className="ml-0.5 text-muted-foreground">
                                                                /{formatMoney4(key.quota)}$
                                                            </span>
                                                        )}
                                                    </Badge>
                                                </div>
