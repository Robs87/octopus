...
// 剩余额度格式化：消除浮点误差（如 10 - 0.002892 = 9.997108000000001）并去掉尾零
function formatRemainQuota(v: number): string {
...
export function ChannelForm({
    ...
    // 额度交互：key 行只读展示"余额"（quota - total_cost），点击后弹出弹窗直接编辑余额。
    // 弹窗内：余额（可编辑）+ 当前消耗（只读），确认后 quota = 新余额 + 当前消耗。
    const [quotaDialogIdx, setQuotaDialogIdx] = useState<number | null>(null);
    const [quotaDraft, setQuotaDraft] = useState('');

    const openQuotaDialog = (idx: number) => {
        const k = formData.keys[idx];
        setQuotaDraft(
            k.quota === undefined && k.total_cost === undefined
                ? ''
                : formatRemainQuota((k.quota ?? 0) - (k.total_cost ?? 0))
        );
        setQuotaDialogIdx(idx);
    };

    const confirmQuotaEdit = () => {
        if (quotaDialogIdx === null) return;
        const v = quotaDraft;
        const num = v === '' ? 0 : Number(v);
        const consumed = formData.keys[quotaDialogIdx]?.total_cost ?? 0;
        const newQuota = Number.isFinite(num) ? Math.max(0, num + consumed) : consumed;
        handleUpdateKey(quotaDialogIdx, { quota: newQuota });
        setQuotaDialogIdx(null);
    };

    // key 行展示的剩余额度（无额度字段时显示空，走 placeholder）
    const remainOfKey = (k: ChannelKeyFormItem) =>
        k.quota === undefined && k.total_cost === undefined
            ? ''
            : formatRemainQuota((k.quota ?? 0) - (k.total_cost ?? 0));
    ...
    const handleAddKey = () => {
        onFormDataChange({
            ...formData,
            keys: [...formData.keys, { enabled: true, channel_key: '', quota: 0 }],
        });
    };
    ...
    const handleRemoveKey = (idx: number) => {
        ...
        if (quotaDialogIdx === idx) setQuotaDialogIdx(null);
        onFormDataChange({ ...formData, keys: next });
    };
    ...
    return (
        <form onSubmit={onSubmit} className="space-y-4 px-1">
                    ...
                    {(formData.keys ?? []).map((k, idx) => (
                        <div key={k.id ?? `new-${idx}`} className="flex items-center gap-2">
                            ...
                            <Input
                                ...
                                onClick={() => openQuotaDialog(idx)}
                                className="rounded-xl w-[77px] cursor-pointer"
                            />
                            <Input
                                type="text"
                                value={k.remark ?? ''}
            ...
            {/* 额度编辑弹窗：点击 key 行余额打开，直接编辑余额、展示当前消耗 */}
            {quotaDialogIdx !== null && formData.keys[quotaDialogIdx] && (
                <Dialog open onOpenChange={(open) => { if (!open) setQuotaDialogIdx(null); }}>
                    <DialogContent className="sm:max-w-sm rounded-xl">
                        <DialogHeader>
                            <DialogTitle>{t('quotaEdit')}</DialogTitle>
                            <DialogDescription>
                                {t('quotaBalance')}: {remainOfKey(formData.keys[quotaDialogIdx]) || '0'}
                            </DialogDescription>
                        </DialogHeader>
                        <div className="space-y-3">
                            <div className="space-y-1.5">
                                <label className="text-sm font-medium text-card-foreground">{t('quotaBalance')}</label>
                                <Input
                                    type="text"
                                    inputMode="decimal"
                                    autoFocus
                                    ...
                                    onChange={(e) => {
                                        ...
                                        // 只允许数字与一个小数点，支持小数额度（如 0.0001、100.5）
                            ...
                            <div className="space-y-1.5">
                                <label className="text-sm font-medium text-card-foreground">{t('quotaConsumed')}</label>
                                <div className="rounded-xl border border-input bg-muted/30 px-3 py-2 text-sm text-muted-foreground">
                                    {formatRemainQuota(formData.keys[quotaDialogIdx].total_cost ?? 0)}
                                </div>
                            </div>
                        </div>
                        <DialogFooter>
                            <Button type="button" variant="ghost" onClick={() => setQuotaDialogIdx(null)}>
                                {t('quotaCancel')}
                            </Button>
                            <Button type="button" onClick={confirmQuotaEdit}>
                                {t('quotaConfirm')}
                            </Button>
                        </DialogFooter>
                    </DialogContent>
