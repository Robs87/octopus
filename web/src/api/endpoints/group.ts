...
export interface GroupUpdateRequest {
    ...
    context_length?: number;              // 仅在上下文长度变更时发送
...
export interface TestGroupResponse {
    ...
    latency_ms?: number;
    status_code?: number;
    model_context_length?: number; // 上游模型上下文长度(tokens)；与分组设置不一致时返回
    message: string;
}

/**
 * 测试分组 Hook
 * 根据分组规则（轮询/随机/加权/故障转移）测试该分组下模型是否可以调用
 *
 * @example
 * const testGroup = useTestGroup();
 * testGroup.mutate(1); // 测试 group_id=1 的分组
 */
export function useTestGroup() {
    ...
    return useMutation({
        mutationFn: async (groupId: number) => {
            return apiClient.post<TestGroupResponse>('/api/v1/group/test', { group_id: groupId });
        },
        onSuccess: (data) => {
            logger.log('分组测试完成:', data);
        },
        onError: (error) => {
            logger.error('分组测试失败:', error);
        },
        // 测试可能触发余额不足自动停用 key/渠道，刷新渠道列表
        onSettled: () => {
