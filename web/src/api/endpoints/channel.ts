...
export type CreateChannelRequest = {
    ...
    enabled?: boolean;
    base_urls: BaseUrl[];
    keys: Array<Pick<ChannelKey, 'enabled' | 'channel_key' | 'quota' | 'remark'>>;
    model: string;
    custom_model?: string;
    proxy?: boolean;
    auto_sync?: boolean;
...
export type UpdateChannelRequest = {
    ...
    match_regex?: string | null;
    // keys diff
    keys_to_add?: Array<Pick<ChannelKey, 'enabled' | 'channel_key' | 'quota' | 'remark'>>;
    keys_to_update?: Array<{ id: number; enabled?: boolean; channel_key?: string; quota?: number; remark?: string }>;
...
export type TestModelRequest = {
    channel_id: number;
    model: string;
    /** true 时逐个测试渠道下所有启用的 key；默认 false 按规则选一个 key */
    test_all_keys?: boolean;
};

/**
 * 单个 key 的测试结果（test_all_keys 模式）
 */
export type TestModelKeyResult = {
    key_id: number;
    key_name: string;
    success: boolean;
    latency_ms: number;
...
/**
 * 渠道模型测试结果
 * test_all_keys 模式下 total/passed/results 有值，success 表示所有 key 均通过
 */
 ...
 * // onSuccess 中 data: { success: true, latency_ms: 320, status_code: 200, message: 'ok' }
 */
export function useTestModel() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: async (data: TestModelRequest) => {
            return apiClient.post<TestModelResponse>('/api/v1/channel/test-model', data);
        },
        onSuccess: (data) => {
            logger.log('渠道模型测试完成:', data);
        },
        onError: (error) => {
            logger.error('渠道模型测试失败:', error);
        },
        // 测试可能产生费用/触发余额不足自动停用 key，刷新渠道列表让金额与状态及时更新
        onSettled: () => {
            queryClient.invalidateQueries({ queryKey: ['channels', 'list'] });
        },
    });
